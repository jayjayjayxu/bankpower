...
企业详情页通过 Java 只读接口查询 MySQL，支持91家企业搜索、月度用电、财务记录及小时负荷分页。首页重点企业、工作台全量队列及算力设施/商品均由数据库读取。工作台按优先级排序并支持分页检索。

所有项目结论均显示为研究情景，不能替代企业工程、电价及授信尽调。数据库密码只允许通过环境变量或已被 Git 忽略的 `.env` 注入，不能写入源码。

## 2026-09-08 网页修复与模拟样本

百旺信三期独立模拟数据集、复算方法和边界见 [修复说明](docs/算电协同模拟与网页修复_20260908.md)。模拟不会覆盖公开事实或将尽调材料标为已取得。
