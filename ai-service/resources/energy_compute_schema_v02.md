...
## 8A. enterprise_public_energy_metric — enterprise public operating metrics

One row is one enterprise metric for a stated report year. Join company_id to enterprise_profile.company_id. This is the current controlled table for imported public operating metrics; do not query the retired `enterprise_operational_statistic_v1` name.

Fields:
- company_id, report_year, metric_code, metric_name, metric_value, metric_unit — entity, period, metric and unit.
- reporting_scope, source_id, source_page, data_type, verification_status, data_version, notes — provenance boundary. `SIMULATED` or `SIMULATED_TEST_ONLY` records are testing scenarios, not operating disclosures.
...
- enterprise_public_energy_metric.company_id = enterprise_profile.company_id
