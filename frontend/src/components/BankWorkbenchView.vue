...
import { amount, decimal, percent, scaled } from '../utils/numbers'
...
const error = ref('')
const data = ref(null)
const saved = (() => { try { return JSON.parse(sessionStorage.getItem('workbench-filter') || '{}') } catch { return {} } })()
const selectedTrack = ref(saved.track || 'ALL')
const query = ref(saved.query || '')
const page = ref(0)
const pageSize = 12
watch([selectedTrack, query], () => {
  page.value = 0
  try { sessionStorage.setItem('workbench-filter', JSON.stringify({ track: selectedTrack.value, query: query.value })) } catch { /* storage may be disabled */ }
})
...
const allItems = computed(() => {
  ...
  return [...power, ...compute].sort((a, b) => (a.businessPriority || 'Z').localeCompare(b.businessPriority || 'Z'))
})
  ...
  <main class="workbench-shell">
    <header class="workbench-header">
      <div class="workbench-topbar">
        <button class="workbench-brand" type="button" @click="emit('back')">
          ...
          <span><strong>EnergyComputeAI</strong><small>银行客户经理工作台</small></span>
        </button>
    ...
    <template v-else>
      <section class="summary-strip" aria-label="工作台概览">
        <article><span>电力客户库</span><strong>{{ data.summary.powerCandidateCount }}</strong><small>{{ data.summary.powerModelledCount }} 家已具备模型快照</small></article>
        <article><span>电力A级线索</span><strong>{{ data.summary.powerPriorityACount }}</strong><small>按当前快照的业务优先级</small></article>
        <article><span>算力项目样本</span><strong>{{ data.summary.computeFocusCustomerCount }}</strong><small>百旺信三期 · 公开项目样本</small></article>
      ...
      <section class="action-grid">
        <article>
          ...
          <button type="button" @click="selectedTrack = 'POWER'">浏览全部电力客户 {{ powerItems.length }} 家 ↓</button>
        </article>
        <article class="compute-action">
          <span>02 · 先尽调什么</span>
          <h2>百旺信云数据中心三期</h2>
          <p>物理边界、投资、PUE与能耗锚点已披露；项目级收入、成本、回款和债务本息仍是获得授信测算的关键缺口。</p>
          <a :href="`${computeSiteUrl}/due-diligence/baiwangxin-phase3`">进入三期尽调状态 →</a><br /><a :href="`${computeSiteUrl}/power-synergy/baiwangxin-phase3`">查看算电协同模拟 →</a>
        </article>
      ...
      <section class="queue-section">
        <header class="queue-heading">
          <div><span>UNIFIED ACTION QUEUE</span><h2>客户与项目机会队列</h2><p>覆盖全部电力客户与算力线索；支持按赛道和名称筛选，点击进入详情。</p></div>
        ...
        <div class="queue-table" role="table" aria-label="客户与项目机会队列">
          ...
          <article v-for="item in visibleItems" :key="item.key" class="queue-row" :class="item.track.toLowerCase()" role="row">
            ...
            <div class="direction-cell" role="cell"><b>{{ item.recommendedProduct || '业务方向待评估' }}</b><p>{{ item.recommendationText || (item.track === 'POWER' ? '尚未形成项目级分析，请先核验企业与项目资料。' : '公开线索与项目尽调样本；需用项目级材料更新测算。') }}</p></div>
            ...
            <div class="metric-cell" role="cell">
              ...
              <template v-else-if="item.track === 'COMPUTE'">
                <span>研究模型</span><strong>NPV {{ amount(scaled(item.npvYuan, 10000)) }} 万元</strong>
                <small>最低 DSCR {{ decimal(item.recommendedMinDscr) }} · 建议贷款 {{ amount(scaled(item.recommendedLoanYuan, 10000)) }} 万元</small>
              </template>
              <template v-else>
          ...
          <div class="queue-tools" style="padding-top:18px"><button :disabled="page === 0" @click="page--">上一页</button><span>共 {{ filteredItems.length }} 条 · {{ totalPages ? page + 1 : 0 }} / {{ totalPages }} 页</span><button :disabled="page + 1 >= totalPages" @click="page++">下一页</button></div>
      ...
      <section v-if="computeItem" class="compute-detail-strip">
        ...
        <dl>
          <div><dt>公开历史投资</dt><dd>{{ amount(scaled(computeItem.referenceHistoricalCapexYuan, 10000)) }} 万元</dd></div>
          <div><dt>公开PUE锚点</dt><dd>{{ decimal(computeItem.referencePue, 3) }}</dd></div>
          <div><dt>公开年电量边界</dt><dd>{{ amount(scaled(computeItem.referenceAnnualEnergyCapKwh, 10000)) }} 万kWh</dd></div>
...
.queue-table { border-top: 1px solid #dae5e8; }.queue-row { display: grid; grid-template-columns: 1.15fr 1.15fr 1fr 1.15fr .92fr; gap: 18px; align-items: start; padding: 19px 8px; border-bottom: 1px solid #e6edef; }.queue-labels { padding-top: 11px; padding-bottom: 9px; color: #8296a0; font-size: 12px; }.queue-row:last-child { border-bottom: 0; }.subject-cell strong { display: block; margin-top: 7px; color: #234860; font-size: 14px; }.subject-cell small { display: block; margin-top: 4px; color: #84949c; font-size: 12px; line-height: 1.5; }.track-tag { display: inline-block; padding: 3px 5px; border-radius: 3px; color: #376d86; background: #edf4f7; font-size: 12px; }.compute .track-tag { color: #2e8278; background: #e7f5f2; }.subject-cell em { display: inline-block; margin: 9px 4px 0 0; padding: 3px 5px; border-radius: 3px; color: #767f86; background: #f0f3f4; font-size: 12px; font-style: normal; }.subject-cell em.priority-a { color: #1f766f; background: #e4f5f1; }.subject-cell em.due { color: #956a1d; background: #fff1d7; }.direction-cell b { color: #31556a; font-size: 12px; }.direction-cell p,.evidence-cell p,.open-cell p { margin: 7px 0 0; color: #718590; font-size: 12px; line-height: 1.65; }.metric-cell span { color: #76919c; font-size: 12px; }.metric-cell strong { display: block; margin: 5px 0; color: #26576a; font-size: 16px; letter-spacing: -.02em; }.metric-cell small { color: #84949c; font-size: 12px; line-height: 1.5; }.source-badge { display: inline-block; padding: 3px 5px; border-radius: 3px; color: #2d7772; background: #e4f5f0; font-size: 12px; }.source-badge.scenario { color: #976a1b; background: #fff1d9; }.source-badge.public { color: #3e7187; background: #eaf2f6; }.source-badge.unknown { color: #6d777d; background: #f0f2f3; }.evidence-cell small { display: block; margin-top: 6px; color: #90a0a7; font-size: 12px; line-height: 1.55; }.open-cell button,.open-cell a { padding: 7px 9px; border: 1px solid #b9d9d6; border-radius: 5px; color: #277a73; background: #f4fbf9; cursor: pointer; font-size: 12px; font-weight: 700; text-decoration: none; }.open-cell button:hover,.open-cell a:hover { background: #e6f5f1; }.empty-state { padding: 28px; color: #83949d; font-size: 12px; text-align: center; }
.compute-detail-strip { display: grid; grid-template-columns: 1fr 1.24fr; gap: 27px; align-items: center; margin-top: 46px; padding: 26px; border: 1px solid #cde1df; border-radius: 9px; background: #edf8f6; }.compute-detail-strip h2 { font-size: 22px; }.compute-detail-strip dl { display: grid; grid-template-columns: repeat(2, 1fr); margin: 0; border: 1px solid #d6e6e4; background: #d6e6e4; }.compute-detail-strip dl div { padding: 12px; background: rgba(255,255,255,.8); }.compute-detail-strip dt { color: #76909a; font-size: 12px; }.compute-detail-strip dd { margin: 6px 0 0; color: #31596a; font-size: 13px; font-weight: 700; }.compute-detail-strip dd.pending { color: #9b7023; }.compute-detail-strip > p { grid-column: 1 / -1; margin: -8px 0 0; color: #6d818b; font-size: 11px; line-height: 1.65; }.workbench-boundary { display: grid; grid-template-columns: 90px 1fr; gap: 16px; padding: 32px 0 50px; color: #728792; font-size: 11px; line-height: 1.7; }.workbench-boundary b { color: #31566b; }
